...
func AutoMigrate(db *sql.DB) error {
    ...
    if _, err := db.Exec(createTransfers); err != nil { return err }
    ...
    if _, err := db.Exec(createReadings); err != nil { return err }
    ...
    if _, err := db.Exec(createAlerts); err != nil { return err }
    ...
    if _, err := db.Exec(createOutbox); err != nil { return err }
...
func (r *PostgresRepo) GetTransfer(ctx context.Context, id string) (*service.Transfer, error) {
    ...
    if err := row.Scan(&t.ID,&t.PalletID,&t.FromLocation,&t.ToLocation,&t.Status,&t.RequestedBy,&approved,&t.CreatedAt,&t.UpdatedAt); err != nil { return nil, err }
    if approved.Valid { t.ApprovedBy = &approved.String }
    return &t, nil
}
...
func (r *PostgresRepo) FlushOutboxAndMark(ctx context.Context, outboxDir string) error {
    ...
    for rows.Next() {
        var e OutboxEvent
        if err := rows.Scan(&e.ID, &e.AggregateType, &e.AggregateID, &e.Payload); err != nil { return err }
        events = append(events, e)
    }
    for _, ev := range events {
        fname := fmt.Sprintf("%s_%s.json", ev.AggregateType, ev.ID)
        path := filepath.Join(outboxDir, fname)
        if err := ioutil.WriteFile(path, []byte(ev.Payload), 0o644); err != nil { return err }
        if _, err := r.DB.ExecContext(ctx, `UPDATE outbox SET published=true WHERE id=$1`, ev.ID); err != nil { return err }
...
func (r *PostgresRepo) ListAlerts(ctx context.Context) ([]service.Alert, error) {
    ...
    for rows.Next() {
        var a service.Alert
        if err := rows.Scan(&a.ID, &a.RoomID, &a.Temp, &a.Level, &a.Message, &a.Created); err != nil { return nil, err }
        res = append(res, a)
    }
    return res, nil
}
